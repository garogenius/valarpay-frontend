...
const EasyLifeSavingsModal: React.FC<EasyLifeSavingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleCreate = () => {
    ...
    const planType: "FLEX_SAVE" | "NATTY_AUTO_SAVE" = hasAmount ? "FLEX_SAVE" : "NATTY_AUTO_SAVE";

    if (hasAmount && Number(amount) <= 0) {
      ErrorToast({
        title: "Validation Error",
    ...
    let durationMonths: number = hasAmount ? 12 : 6; // Default based on plan type
    ...
    // For NATTY_AUTO_SAVE, must be 6, 12, or 18 months
    if (planType === "NATTY_AUTO_SAVE" && ![6, 12, 18].includes(durationMonths)) {
    ...
    // For NATTY_AUTO_SAVE, require minimum ₦100,000
    if (planType === "NATTY_AUTO_SAVE" && (!hasAmount || Number(amount) < 100000)) {
      ErrorToast({
        title: "Validation Error",
    ...
    const payload = {
      type: planType,
      name: name.trim(),
      description: `Easy-life savings plan for ${name.trim()}`,
      goalAmount: hasAmount ? Number(amount) : 100000, // Default to 100k for NATTY_AUTO_SAVE
              ...
              <div>
                ...
                <div className="rounded-lg bg-bg-800 dark:bg-bg-1000 divide-y divide-white/10">
                  <label className="flex items-start justify-between px-3 py-2.5 text-white cursor-pointer hover:bg-bg-900 dark:hover:bg-bg-950 transition-colors first:rounded-t-lg">
                    ...
                    <input type="radio" name="fundingModeEasy" checked={mode === 'manual'} onChange={()=> setMode("manual")} className="mt-0.5 w-4 h-4 accent-[#FF6B2C]" />
                  </label>
                  <label className="flex items-start justify-between px-3 py-2.5 text-white cursor-pointer hover:bg-bg-900 dark:hover:bg-bg-950 transition-colors last:rounded-b-lg">
                    ...
                    <input type="radio" name="fundingModeEasy" checked={mode === 'auto'} onChange={()=> setMode("auto")} className="mt-0.5 w-4 h-4 accent-[#FF6B2C]" />
                  </label>
                </div>
              </div>
